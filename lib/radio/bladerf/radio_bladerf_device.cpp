...
bool radio_bladerf_device::open(const std::string& device_address)
{
  ...
  if (getenv("AGC") == nullptr) {
    fmt::print("Setting manual Rx gain...\n");
...
bool radio_bladerf_device::set_sync_source(const radio_configuration::clock_sources& config)
{
  if (config.clock == radio_configuration::clock_sources::source::EXTERNAL) {
    ...
    if (enabled) {
      fmt::print("Disabling reference clock input...\n");
                                       ...
                                       const radio_bladerf_tx_stream::stream_description& description)
{
  ...
  fmt::print("Error: failed to create transmit stream {}. {}.\n", description.id, stream->get_error_message().c_str());
  return nullptr;
}
                                       ...
                                       const radio_bladerf_rx_stream::stream_description& description)
{
  std::unique_ptr<radio_bladerf_rx_stream> stream =
      std::make_unique<radio_bladerf_rx_stream>(device, description, notifier);

  if (stream->is_successful()) {
    return stream;
  }

  fmt::print("Error: failed to create receive stream {}. {}.\n", description.id, stream->get_error_message().c_str());
  return nullptr;
}

bool radio_bladerf_device::set_tx_rate(double& actual_rate, double rate)
{
  fmt::print("Setting Tx Rate to {} MHz...\n", to_MHz(rate));
  ...
  fmt::print("Set Tx sampling rate to {} MHz and filter BW to {} MHz\n", to_MHz(rf_actual_rate), to_MHz(rf_actual_bw));
  return true;
}

bool radio_bladerf_device::set_rx_rate(double& actual_rate, double rate)
{
  fmt::print("Setting Rx Rate to {} MHz...\n", to_MHz(rate));
  ...
  status = bladerf_set_bandwidth(device, BLADERF_RX_X1, static_cast<bladerf_bandwidth>(rate * 0.9), &rf_actual_bw);
  if (status != 0) {
    on_error("bladerf_set_bandwidth() failed - {}", bladerf_strerror(status));
    return false;
  }

  fmt::print("Set Rx sampling rate to {} MHz and filter BW to {:.2f} MHz\n", to_MHz(rf_actual_rate), to_MHz(rf_actual_bw));
  return true;
}

bool radio_bladerf_device::set_tx_gain(unsigned ch, double gain)
{
  fmt::print("Setting channel {} Tx gain to {:.2f} dB...\n", ch, gain);
...
{
  fmt::print("Setting channel {} Rx gain to {:.2f} dB...\n", ch, gain);
...
{
  fmt::print("Setting channel {} Tx frequency to {} MHz...\n", ch, to_MHz(config.center_frequency_hz));
...
{
  fmt::print("Setting channel {} Rx frequency to {} MHz...\n", ch, to_MHz(config.center_frequency_hz));
...
baseband_gateway_timestamp radio_bladerf_device::get_time_now()
{
  ...
  if (status != 0) {
    fmt::print("Failed to get current Rx timestamp: {}", bladerf_strerror(status));
  }
