...
bool radio_session_bladerf_impl::set_tx_freq(unsigned stream_id, double center_freq_Hz)
{
  bool result = true;
  for (unsigned i_port = 0, end = tx_port_map.size(); i_port != end; ++i_port) {
    result &= set_tx_freq(i_port, {.center_frequency_Hz = center_freq_Hz, .lo_frequency_Hz = 0.0});
  }
  return result;
}

bool radio_session_bladerf_impl::set_rx_freq(unsigned stream_id, double center_freq_Hz)
{
  bool result = true;
  for (unsigned i_port = 0, end = rx_port_map.size(); i_port != end; ++i_port) {
    result &= set_rx_freq(i_port, {.center_frequency_Hz = center_freq_Hz, .lo_frequency_Hz = 0.0});
  }
  return result;
}
