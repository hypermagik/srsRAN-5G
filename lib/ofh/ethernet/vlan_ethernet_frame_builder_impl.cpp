...
units::bytes vlan_frame_builder_impl::get_header_size(const vlan_frame_params& eth_params) const
{
  units::bytes header_size = ETH_HEADER_SIZE;
  if (eth_params.tci > 0) {
    header_size += ETH_VLAN_TAG_SIZE;
  }
  return header_size;
}
