...
void srsran::srsvec::convert(span<int8_t> z, span<const cf_t> x, float scale)
{
...
void srsran::srsvec::convert(span<int8_t> z, span<const cf_t> x0, span<const cf_t> x1, float scale)
{
  assert(x0.size() == x1.size());
  assert(2 * x0.size() + 2 * x1.size() == z.size());

  convert_fb_simd(reinterpret_cast<const float*>(x0.data()), reinterpret_cast<const float*>(x1.data()), z.data(), scale, z.size());
}

void srsran::srsvec::convert(span<cf_t> z, span<const int8_t> x, float scale)
{
...
void srsran::srsvec::convert(span<cf_t> z0, span<cf_t> z1, span<const int8_t> x, float scale)
{
  assert(z0.size() == z1.size());
  assert(x.size() == 2 * z0.size() + 2 * z1.size());

  convert_bf_simd(x.data(), reinterpret_cast<float*>(z0.data()), reinterpret_cast<float*>(z1.data()), scale, x.size());
}
...
void srsran::srsvec::convert(span<int16_t> z, span<const cf_t> x0, span<const cf_t> x1, float scale)
{
  assert(x0.size() == x1.size());
  assert(2 * x0.size() + 2 * x1.size() == z.size());

  convert_fi_simd(reinterpret_cast<const float*>(x0.data()), reinterpret_cast<const float*>(x1.data()), z.data(), scale, z.size());
}
...
void srsran::srsvec::convert(span<cf_t> z0, span<cf_t> z1, span<const int16_t> x, float scale)
{
  assert(z0.size() == z1.size());
  assert(x.size() == 2 * z0.size() + 2 * z1.size());

  convert_if_simd(x.data(), reinterpret_cast<float*>(z0.data()), reinterpret_cast<float*>(z1.data()), scale, x.size());
}
